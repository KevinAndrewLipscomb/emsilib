    ...
    private static class Static
      ...
      public const int TCCI_SRR_REPLACEMENT_ROW_INDICATOR = 12;
        ...
        private void BindOverview
          ...
          object target,
          string where_parm = k.EMPTY,
          string and_parm = k.EMPTY
          )
          {
            string cmdText;
            string where_clause;
            ...
            else
            {
                ...
                if (and_parm.Length > 0)
                {
                    where_clause += " and " + and_parm;
                }
            }
            cmdText = Static.FULL_REQUEST_REVIEW_APPROVE_SELECT_FROM_EXPRESSION + where_clause + " order by " + order_by_field_name;
        ...
        public decimal ActualValueOf(string master_id)
        {
            decimal result;
            Open();
            using var my_sql_command = new MySqlCommand("select actual_value from emsof_request_master where id = " + master_id, connection);
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("insert into emsof_purchase_payment" + " set master_id = " + request_id + " , date_of = \"" + date_of_payment.ToString("yyyy-MM-dd") + "\"" + " , method_code = " + method_code + " , amount = " + amount.ToString() + " , note = \"" + note + "\""), connection);
        ...
        {
            string result;
            result = k.Safe(((e_item) as DataGridItem).Cells[Static.TCCI_AFFILIATE_NUM].Text, k.safe_hint_type.NUM);
            return result;
        }
        ...
        public void Approve(string master_id, uint next_status, string user_kind, approval_timestamp_column_type approval_timestamp_column, decimal amount_to_return_to_county)
        {
            ...
            if (approval_timestamp_column != approval_timestamp_column_type.NONE)
            {
                ...
                if (next_status == 8)
                {
                    cmdText += " , be_reopened_after_going_to_state = FALSE";
                }
        ...
        public Queue ArchiveMatured()
        {
            ...
            dr.Close();
            if (target_id_clause.Length > 0)
            {
                using var my_sql_command_2 = new MySqlCommand(db_trail.Saved("update emsof_request_master" + " set emsof_request_master.status_code = 15" + " where id in (" + target_id_clause + ")"), connection);
        ...
        public bool BeDeadlineExempt(string master_id)
        {
            bool result;
            Open();
            using var my_sql_command = new MySqlCommand("select be_deadline_exempt from emsof_request_master where id = " + master_id, connection);
        ...
        public bool BeRequestsEligibleForUnrejectionByRegionDictatedAppropriation(string region_dictated_appropriation_id)
        {
            bool result;
            Open();
            using var my_sql_command = new MySqlCommand("select count(*) from emsof_request_master" + " join county_dictated_appropriation" + " on (county_dictated_appropriation.id=emsof_request_master.county_dictated_appropriation_id)" + " where status_code = 11" + " and region_dictated_appropriation_id = " + region_dictated_appropriation_id, connection);
            result = "0".CompareTo(my_sql_command.ExecuteScalar().ToString()) < 0;
            Close();
            return result;
        }
        ...
        public bool BeValidCountyApprovalTimestampOf(string master_id, out DateTime timestamp)
        {
            ...
            Open();
            using var my_sql_command = new MySqlCommand("select county_approval_timestamp from emsof_request_master where id = " + master_id, connection);
        ...
        public bool BeValidRegionalExecDirApprovalTimestampOf(string master_id, out DateTime timestamp)
        {
            ...
            timestamp = DateTime.MinValue;
            Open();
            using var my_sql_command = new MySqlCommand("select regional_director_approval_timestamp from emsof_request_master where id = " + master_id, connection);
            dr = my_sql_command.ExecuteReader();
            dr.Read();
            if (dr.IsDBNull(0))
            {
        ...
        public bool BeValidRegionalPlannerApprovalTimestampOf(string master_id, out DateTime timestamp)
        {
            ...
            timestamp = DateTime.MinValue;
            Open();
            using var my_sql_command = new MySqlCommand("select regional_planner_approval_timestamp from emsof_request_master where id = " + master_id, connection);
            dr = my_sql_command.ExecuteReader();
            dr.Read();
            if (dr.IsDBNull(0))
            {
        ...
        public bool BeValidStateApprovalTimestampOf(string master_id, out DateTime timestamp)
        {
            ...
            timestamp = DateTime.MinValue;
            Open();
            using var my_sql_command = new MySqlCommand("select state_approval_timestamp from emsof_request_master where id = " + master_id, connection);
            dr = my_sql_command.ExecuteReader();
            dr.Read();
            if (dr.IsDBNull(0))
            {
        ...
        public void BindEquipmentProcurementOverview(string fy_id, string sort_order, bool be_order_ascending, object target, bool do_log)
        {
            ...
            if (do_log)
            {
                using var my_sql_command_1 = new MySqlCommand(db_trail.Saved("replace indicator_equipment_quantities (fiscal_year_id,description,quantity)" + " select fiscal_year_id" + " , description" + " , sum(actual_quantity) as quantity" + metric_from_where_clause + " group by eligible_provider_equipment_list.description"), connection);
            ...
            using var my_sql_command_2 = new MySqlCommand("select code,description" + " , sum(actual_quantity) as quantity" + " , sum(actual_emsof_ante) as emsof_part" + " , sum(actual_subtotal_cost) as overall_cost" + " , (sum(actual_subtotal_cost)/sum(actual_quantity)) as avg_unit_cost" + metric_from_where_clause + " group by eligible_provider_equipment_list.code" + " order by " + sort_order, connection);
        ...
        public void BindProofsOfPayment(string request_id, object target)
        {
            ...
            // column 3
            // column 4
            using var my_sql_command = new MySqlCommand("select id" + " , DATE_FORMAT(date_of,\"%Y-%m-%d\") as date_of" + " , description as method" + " , amount" + " , note" + " from emsof_purchase_payment" + " join payment_proof_method_code_description_map" + " on (payment_proof_method_code_description_map.code=emsof_purchase_payment.method_code)" + " where master_id = " + request_id + " order by date_of asc", connection);
            ((target) as DataGrid).DataSource = my_sql_command.ExecuteReader();
            ((target) as DataGrid).DataBind();
            ((MySqlDataReader)(((DataGrid)(target)).DataSource)).Close();
            Close();
        ...
        public string CountyCodeOfMasterId(string master_id)
        {
            string result;
            Open();
            using var my_sql_command = new MySqlCommand("select county_code" + " from emsof_request_master" + " join county_dictated_appropriation" + " on (county_dictated_appropriation.id=emsof_request_master.county_dictated_appropriation_id)" + " join region_dictated_appropriation" + " on (region_dictated_appropriation.id=county_dictated_appropriation.region_dictated_appropriation_id)" + " where emsof_request_master.id = " + master_id, connection);
        ...
        public string CountyDictumIdOf(string master_id)
        {
            string result;
            Open();
            using var my_sql_command = new MySqlCommand("select county_dictated_appropriation_id" + " from emsof_request_master" + " where id = " + master_id, connection);
            result = my_sql_command.ExecuteScalar().ToString();
            Close();
            return result;
        }
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("delete from emsof_purchase_payment where id = " + id), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("update emsof_request_master" + " join county_dictated_appropriation" + " on (county_dictated_appropriation.id=emsof_request_master.county_dictated_appropriation_id)" + " set emsof_request_master.status_code = " + next_status.ToString() + " where emsof_request_master.id = " + master_id), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        public string DetailText(string master_id)
        {
            ...
            detail_text = k.EMPTY;
            Open();
            using var my_sql_command = new MySqlCommand("select priority" + " , description" + " , make_model" + " , place_kept" + " , if(be_refurbished,\"REFURB\",\"NEW\") as new_or_refurb" + " , quantity" + " , concat(\"$\",format(unit_cost,2)) as formatted_unit_cost" + " , concat(\"$\",format(emsof_ante,2)) as formatted_emsof_ante" + " from emsof_request_detail" + " join eligible_provider_equipment_list on (eligible_provider_equipment_list.code=emsof_request_detail.equipment_code)" + " where master_id = " + master_id + " order by priority", connection);
            dr = my_sql_command.ExecuteReader();
            while (dr.Read())
            {
        ...
        public Queue DeployCompleted()
        {
            ...
            id_q = new Queue();
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("update emsof_request_master set status_code = 14 where status_code = 13"), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
            result = id_q;
            return result;
        ...
        public decimal EmsofAnteOf(string master_id)
        {
            decimal result;
            Open();
            using var my_sql_command = new MySqlCommand("select value from emsof_request_master where id = " + master_id, connection);
            result = (decimal)(my_sql_command.ExecuteScalar());
            Close();
            return result;
        }
        ...
        public decimal EmsofAnteOfItem(string master_id, string priority)
        {
            decimal result;
            Open();
            using var my_sql_command = new MySqlCommand("select emsof_ante" + " from emsof_request_detail" + " where master_id = " + master_id + " and priority = " + priority, connection);
            result = (decimal)(my_sql_command.ExecuteScalar());
            Close();
            return result;
        }
        ...
        public string EquipmentCodeOf(string master_id, string priority)
        {
            string result;
            Open();
            using var my_sql_command = new MySqlCommand("select equipment_code" + " from emsof_request_detail" + " where master_id = " + master_id + " and priority = " + priority, connection);
            result = my_sql_command.ExecuteScalar().ToString();
            Close();
            return result;
        }
        ...
        public Queue FailUncompleted()
        {
            ...
            try {
                using var my_sql_command_1 = new MySqlCommand("select id from emsof_request_master where status_code < 10", connection, transaction);
                dr = my_sql_command_1.ExecuteReader();
                while (dr.Read())
                {
                    id_q.Enqueue(dr["id"]);
                ...
                dr.Close();
                using var my_sql_command_2 = new MySqlCommand(db_trail.Saved("update emsof_request_master set status_code = 16 where status_code < 10"), connection, transaction);
                my_sql_command_2.ExecuteNonQuery();
                transaction.Commit();
            }
            catch {
        ...
        public Queue FailUnfinalized()
        {
            ...
            transaction = connection.BeginTransaction();
            try {
                using var my_sql_command_1 = new MySqlCommand("select id from emsof_request_master where status_code < 3", connection, transaction);
                dr = my_sql_command_1.ExecuteReader();
                while (dr.Read())
                {
                    id_q.Enqueue(dr["id"]);
                ...
                dr.Close();
                using var my_sql_command_2 = new MySqlCommand(db_trail.Saved("update emsof_request_master set status_code = 16 where status_code < 3"), connection, transaction);
                my_sql_command_2.ExecuteNonQuery();
                transaction.Commit();
            }
            catch {
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("update emsof_request_master set status_code = 3 where id = " + master_id), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("update emsof_request_master set be_deadline_exempt = FALSE where id = " + master_id), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("START TRANSACTION;" + " update emsof_request_master" + " set status_code = 2" + " , be_deadline_exempt = TRUE" + " , be_reopened_after_going_to_state = (" + be_reopened_after_going_to_state.ToString() + " or be_reopened_after_going_to_state) where id = " + master_id + ";" + " update emsof_request_detail" + " set status_code = 1" + " , invoice_designator = \"\"" + " , actual_quantity = 0" + " , actual_subtotal_cost = 0" + " , actual_emsof_ante = 0" + " where master_id = " + master_id + ";" + " delete from emsof_purchase_payment where master_id = " + master_id + ";" + " COMMIT"), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        {
            string result;
            result = k.Safe(((e_item) as DataGridItem).Cells[Static.TCCI_FISCAL_YEAR_DESIGNATOR].Text, k.safe_hint_type.ALPHANUM);
            return result;
        }
        ...
        public bool HasWishList(string master_id)
        {
            bool result;
            Open();
            using var my_sql_command = new MySqlCommand("select has_wish_list from emsof_request_master where id = " + master_id, connection);
            result = "1" == my_sql_command.ExecuteScalar().ToString();
            Close();
            return result;
        }
        ...
        {
            string result;
            result = k.Safe(((e_item) as DataGridItem).Cells[Static.TCCI_ID].Text, k.safe_hint_type.NUM);
            return result;
        }
        ...
        {
            string result;
            result = k.Safe(((e_item) as DataGridItem).Cells[Static.TCCI_PROOF_OF_PAYMENT_ID].Text, k.safe_hint_type.NUM);
            return result;
        }
        ...
        public decimal LeftoverOrShortageOf(object e_item)
        {
            decimal result;
            string safe_string;
            safe_string = k.Safe(((e_item) as DataGridItem).Cells[Static.TCCI_LEFTOVER_OR_SHORTAGE].Text, k.safe_hint_type.REAL_NUM_INCLUDING_NEGATIVE);
            if (safe_string.Length == 0)
            {
                result = 0;
            }
            else
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("update emsof_request_master set status_code = 16 where id = " + master_id), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("update emsof_request_master" + " join county_dictated_appropriation" + " on (county_dictated_appropriation.id=emsof_request_master.county_dictated_appropriation_id)" + " join region_dictated_appropriation" + " on (region_dictated_appropriation.id=county_dictated_appropriation.region_dictated_appropriation_id)" + " join state_dictated_appropriation" + " on (state_dictated_appropriation.id=region_dictated_appropriation.state_dictated_appropriation_id)" + " set status_code = " + next_status.ToString() + " where region_code = " + region_code + " and amendment_num = " + amendment_num_string + " and status_code = " + current_status.ToString()), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        public uint NumRequestsInStateExportBatch(uint status, string amendment_num_string, string region_code)
        {
            uint result;
            Open();
            using var my_sql_command = new MySqlCommand("select count(*)" + " from emsof_request_master" + " join county_dictated_appropriation" + " on (county_dictated_appropriation.id=emsof_request_master.county_dictated_appropriation_id)" + " join region_dictated_appropriation" + " on (region_dictated_appropriation.id=county_dictated_appropriation.region_dictated_appropriation_id)" + " join state_dictated_appropriation" + " on (state_dictated_appropriation.id=region_dictated_appropriation.state_dictated_appropriation_id)" + " where status_code = " + status.ToString() + " and value > 0" + " and amendment_num = " + amendment_num_string + " and region_code = " + region_code, connection);
        ...
        public string PasswordResetEmailAddressOf(object e_item)
          {
          return k.Safe(((e_item) as DataGridItem).Cells[Static.TCCI_PASSWORD_RESET_EMAIL_ADDRESS].Text, k.safe_hint_type.EMAIL_ADDRESS);
          }
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("update emsof_request_master" + " set actual_value = (select sum(actual_emsof_ante) from emsof_request_detail where master_id = " + master_id + ")" + " where id = " + master_id), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        {
            string result;
            result = k.Safe(((e_item) as DataGridItem).Cells[Static.TCCI_SERVICE_ID].Text, k.safe_hint_type.NUM);
            return result;
        }
        ...
        public string ServiceIdOfMasterId(string master_id)
        {
            string result;
            Open();
            using var my_sql_command = new MySqlCommand("select service_id" + " from emsof_request_master" + " join county_dictated_appropriation" + " on (county_dictated_appropriation.id=emsof_request_master.county_dictated_appropriation_id)" + " where emsof_request_master.id = " + master_id, connection);
            result = my_sql_command.ExecuteScalar().ToString();
            Close();
            return result;
        }
        ...
        {
            string result;
            result = k.Safe(((e_item) as DataGridItem).Cells[Static.TCCI_SERVICE_NAME].Text, k.safe_hint_type.ORG_NAME);
            return result;
        }
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("update emsof_request_detail" + " set invoice_designator = \"" + invoice_designator + "\"" + " , actual_quantity = " + quantity + " , actual_subtotal_cost = " + subtotal_cost + " , actual_emsof_ante = " + emsof_ante.ToString() + " , status_code = " + item_status_code.ToString() + " where master_id = " + master_id + " and priority = " + priority), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("update emsof_request_master" + " set has_wish_list = " + value.ToString() + " where id = " + master_id), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        {
            string result;
            result = k.Safe(((e_item) as DataGridItem).Cells[Static.TCCI_SPONSOR_COUNTY_ID].Text, k.safe_hint_type.NUM);
            return result;
        }

        public string SponsorCountyEmailAddressOf(object e_item)
          {
          return k.Safe(((e_item) as DataGridItem).Cells[Static.TCCI_SPONSOR_COUNTY_EMAIL_ADDRESS].Text, k.safe_hint_type.EMAIL_ADDRESS);
          }

        public string SponsorCountyNameOf(object e_item)
        {
            string result;
            result = k.Safe(((e_item) as DataGridItem).Cells[Static.TCCI_SPONSOR_COUNTY_NAME].Text, k.safe_hint_type.ALPHA);
            return result;
        }
        ...
        public string SponsorRegionNameOf(string master_id)
        {
            string result;
            Open();
            using var my_sql_command = new MySqlCommand("select name" + " from emsof_request_master" + " join county_dictated_appropriation" + " on (county_dictated_appropriation.id=emsof_request_master.county_dictated_appropriation_id)" + " join region_dictated_appropriation" + " on (region_dictated_appropriation.id=county_dictated_appropriation.region_dictated_appropriation_id)" + " join state_dictated_appropriation" + " on (state_dictated_appropriation.id=region_dictated_appropriation.state_dictated_appropriation_id)" + " join region_code_name_map on (region_code_name_map.code=state_dictated_appropriation.region_code)" + " where emsof_request_master.id = " + master_id, connection);
            result = my_sql_command.ExecuteScalar().ToString();
            Close();
            return result;
        }
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand("select status_code from emsof_request_master where id = '" + k.Safe((e_item as DataGridItem).Cells[Static.TCCI_ID].Text, k.safe_hint_type.NUM) + "'",connection);
            var status_code_of = uint.Parse(my_sql_command.ExecuteScalar().ToString());
            Close();
        ...
        public decimal SumOfActualCostsOfRequestItems(string request_id)
        {
            ...
            Open();
            using var my_sql_command = new MySqlCommand("select sum(actual_subtotal_cost) from emsof_request_detail where master_id = " + request_id, connection);
            sum_of_actual_costs_of_request_items_obj = my_sql_command.ExecuteScalar();
            Close();
        ...
        public decimal SumOfActualEmsofAntesOfOtherRequestItems
          ...
          Open();
          using var my_sql_command = new MySqlCommand("select sum(actual_emsof_ante) from emsof_request_detail where master_id = '" + request_id + "' and priority <> '" + priority + "'", connection);
          sum_of_actual_emsof_antes_of_request_items_obj = my_sql_command.ExecuteScalar();
          Close();
        ...
        public decimal SumOfProvenPaymentsOfRequest(string request_id)
        {
            ...
            Open();
            using var my_sql_command = new MySqlCommand("select sum(amount) from emsof_purchase_payment where master_id = " + request_id, connection);
            sum_of_proven_payments_of_request_obj = my_sql_command.ExecuteScalar();
            Close();
        ...
        internal Queue SusceptibleTo(milestone_type milestone)
        {
            ...
            Open();
            using var my_sql_command = new MySqlCommand("select id from emsof_request_master where " + where_clause, connection);
            dr = my_sql_command.ExecuteReader();
            while (dr.Read())
            {
                id_q.Enqueue(dr["id"]);
        ...
        {
            uint result;
            result = Static.TCCI_APPROPRIATION;
            return result;
        }

        public uint TcciOfCountyEmailAddress()
          {
          return Static.TCCI_SPONSOR_COUNTY_EMAIL_ADDRESS;
          }

        public uint TcciOfId()
        {
            uint result;
            result = Static.TCCI_ID;
            return result;
        }
        ...
        {
            uint result;
            result = Static.TCCI_FULL_REQUEST_ACTUALS;
            return result;
        }

        public uint TcciOfFullRequestAllowableCost()
          {
          return Static.TCCI_FULL_REQUEST_ALLOWABLE_COST;
          }

        public uint TcciOfFullRequestDetail()
          {
          return Static.TCCI_FULL_REQUEST_DETAIL;
          }

        public uint TcciOfFullRequestPriority()
        {
            uint result;
            result = Static.TCCI_FULL_REQUEST_PRIORITY;
            return result;
        }
        ...
        {
            uint result;
            result = Static.TCCI_PASSWORD_RESET_EMAIL_ADDRESS;
            return result;
        }
        ...
        {
            uint result;
            result = Static.TCCI_SERVICE_NAME;
            return result;
        }
        ...
        {
            uint result;
            result = Static.TCCI_LEFTOVER_OR_SHORTAGE;
            return result;
        }
        ...
        {
            uint result;
            result = Static.TCCI_SRR_REPLACEMENT_ROW_INDICATOR;
            return result;
        }
        ...
        {
            uint result;
            result = Static.TCCI_STATUS_CODE;
            return result;
        }
        ...
        {
            uint result;
            result = Static.TCCI_STATUS_DESCRIPTION;
            return result;
        }
        ...
        internal string RegionCodeOfMasterId(string master_id)
          ...
          var region_code_of_master_id = my_sql_command.ExecuteScalar().ToString();
          Close();
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("update emsof_request_master" + " set status_code = status_code - 1" + " where id = \"" + master_id + "\""), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        {
            Open();
            using var my_sql_command = new MySqlCommand(db_trail.Saved("update emsof_request_master" + " join county_dictated_appropriation" + " on (county_dictated_appropriation.id=emsof_request_master.county_dictated_appropriation_id)" + " set emsof_request_master.status_code = 2" + " where emsof_request_master.id = " + master_id), connection);
            my_sql_command.ExecuteNonQuery();
            Close();
        }
        ...
        public Queue Withdraw(string master_id)
          ...
          Open();
          using var my_sql_command_1 = new MySqlCommand("select attachment_key from emsof_request_detail where master_id = '" + master_id + "' and attachment_key is not null",connection);
          ...
          dr.Close();
          using var my_sql_command_2 = new MySqlCommand(db_trail.Saved("START TRANSACTION;" + "delete from emsof_request_detail where master_id = " + master_id + ";" + "update emsof_request_master" + " set status_code = 12" + " , value = 0" + " , num_items = 0" + " , shortage = 0" + " , has_wish_list = FALSE" + " , actual_value = 0" + " , be_deadline_exempt = FALSE" + " , be_reopened_after_going_to_state = FALSE" + " where id = " + master_id + ";" + "COMMIT;"), connection);
          my_sql_command_2.ExecuteNonQuery();
          Close();
